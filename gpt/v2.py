...
block_size = 256 # context len for predictions
max_iters = 5000
eval_interval = 500
learning_rate = 3e-4
...
class Head(nn.Module):
    ...
    def forward(self, x):
        B,T,C = x.shape
        k = self.key(x) # (B,T,C)
        q = self.value(x) # (B,T,C)
        # compute attention scores ("affinities")
        wei = q @ k.transpose(-2,-1) * C**-0.5 # (B,T,C) @ (B,C,T) -> (B,T,T)
        wei = wei.masked_fill(self.tril[:T,:T]==0, float('-inf')) # (B,T,T)
        wei = F.softmax(wei, dim=-1) #(B,T,T)
        wei = self.dropout(wei)
        # perform the weight agg of the values
...
class FeedForward(nn.Module):
    """ simple linear layer followerd by a non-linearity """
...
for iter in tqdm(range(max_iters)):
    ...
    # every once in a while eval loss on train/val sets
